...
import { HeaderI18n } from "@/components/marketing/header-navigation/header-i18n";

/**
 * Hero Split Image Component with i18n support
 * Hero section พร้อมรองรับหลายภาษา
 * ใช้ translations จาก next-intl และ IBM Plex Sans Thai font
 */
export const HeroSplitImageI18n = () => {
    const t = useTranslations('hero');

    return (
        <Fragment>
            <HeaderI18n className="bg-primary" />
                ...
                <div className="mx-auto flex w-full max-w-container items-center px-4 md:px-8">
                    <div className="flex flex-col items-start md:max-w-3xl lg:w-1/2 lg:pr-8">
                        <a href="#" className="rounded-[10px] outline-focus-ring focus-visible:outline-2 focus-visible:outline-offset-2">
                            <BadgeGroup className="hidden md:flex" size="lg" addonText={t('badge')} iconTrailing={ArrowRight} theme="modern" color="brand">
                                {t('badgeText')}
                            </BadgeGroup>
                            <BadgeGroup className="md:hidden" size="md" addonText={t('badge')} iconTrailing={ArrowRight} theme="modern" color="brand">
                                {t('badgeText')}
                            </BadgeGroup>
                        </a>
